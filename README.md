...
5. In the project's `test` config, under the `options` property, remove the option for `"passWithNoTests": true` that is generated by default. In the same `options` section, add `"runInBand": true` - currently with limited resources on the build server, running the tests in parallel is a heavy load on the server, if we get more resources later, this can be removed. For local testing, to speed things up, add the `-i=false` flag to `ng test` in order to run in parallel

6. Run `ng test components-library-tag`
